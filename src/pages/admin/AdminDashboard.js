  ...
  const handleSubmitPackage = async (e) => {
    ...
    try {
      ...
      if (editingPackage) {
        ...
        await api.put(
          `${API_ENDPOINTS.ADMIN_PACKAGES}/${editingPackage._id}`,
          form,
          {
      ...
      >
        <div className="p-4 md:p-8 max-w-2xl mx-auto">
          ...
          <form onSubmit={handleSubmitPackage} className="space-y-8">
            <Input
              label="Title"
              placeholder="eg: Kerala Summer Trip"
            ...
            <div>
              <h3 className="text-xl font-bold text-primary mb-4">
                Included Services (No Extra Charges)
              </h3>
              <div className="flex flex-col md:flex-row gap-2 mb-4">
                <Input
                  label="Service Name"
                  value={includedServiceName}
                  onChange={(e) => setIncludedServiceName(e.target.value)}
                />
                <Button
                  type="button"
            ...
            <div>
              <h3 className="text-xl font-bold text-primary mb-4">
                Optional Services (Extra Charges)
              </h3>
              <div className="flex flex-col md:flex-row gap-2 mb-4">
                <Input
                  label="Service Name"
                  value={optionalServiceName}
                  onChange={(e) => setOptionalServiceName(e.target.value)}
                />
                <Input
                  label="Service Price"
                  type="number"
                  value={optionalServicePrice}
                  onChange={(e) => setOptionalServicePrice(e.target.value)}
                />
                <Button
                  type="button"
                  onClick={handleAddOptionalService}
                  className="w-full md:w-auto"
                >
                  Add
                </Button>
                ...
                {formData.optionalServices.length === 0 ? (
                  ...
                  formData.optionalServices.map((service, index) => (
                    ...
                    >
                      ...
                      <button
                        type="button"
                        onClick={() => handleRemoveOptionalService(index)}
                        className="text-red-500 text-xs hover:text-red-700"
                      >
                        Remove
                      </button>
